...
// Variables
const rootDomain = "jhoy.biz.id"; // Ganti dengan domain utama kalian
const serviceName = "vpn"; // Ganti dengan nama workers kalian
const apiKey = "2DRdH3SwkDI4YBXJ3452ICXv_vcCIIUmwZh4X5ic"; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
const apiEmail = "fcbanyar@gmail.com"; // Ganti dengan email yang kalian gunakan
const accountID = "c213eb156b24f89f674e1b23766783e9"; // Ganti dengan Account ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
const zoneID = "b93a161744498441ff34d7a870ccacb9"; // Ganti dengan Zone ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
...
const KV_PROXY_URL = "https://raw.githubusercontent.com/liuuufey/ProxyList/refs/heads/main/kvProxyList.json";
const PROXY_BANK_URL = "https://raw.githubusercontent.com/liuuufey/ProxyList/refs/heads/main/ProxyList.txt";
const DNS_SERVER_ADDRESS = "8.8.8.8";
const DNS_SERVER_PORT = 53;
const PROXY_HEALTH_CHECK_API = "https://id1.foolvpn.me/api/v1/check";
const CONVERTER_URL = "https://api.foolvpn.me/convert";
const DONATE_LINK = "https://trakteer.id/jhoy_id/tip";
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    document.setTitle("Welcome to <span class='text-blue-500 font-semibold'>jhoy VPN</span>");
...
<!DOCTYPE html><html lang="en" id="html" class="scroll-auto scrollbar-hide dark">  
<head>
    <meta charset="UTF-8" />
  ...
  <body class="bg-white dark:bg-neutral-800/70 text-neutral-800 dark:text-neutral-200 font-sans">

<!-- Navbar -->
<nav class="w-full border-b border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800/70 px-4 py-3 flex justify-between items-center">
  <!-- Logo link Telegram -->
  <a href="https://t.me/liuuufey" target="_blank" 
     class="flex items-center gap-2 text-lg font-bold hover:text-indigo-500 transition">
    <span class="text-indigo-500">🌐</span> jhoy VPN
  </a>

  <div class="flex items-center gap-2">
    <!-- Menu Negara -->
    <div class="relative">
      <button id="btn-country-menu" class="px-3 py-2 rounded-md bg-neutral-200 dark:bg-neutral-700/70 hover:bg-neutral-300 dark:hover:bg-neutral-600/70">
        ☰ Menu
      </button>

      <!-- Notifikasi memanjang -->
      <div id="notification-badge" 
           class="absolute top-full left-0 mt-2 bg-neutral-800/70 text-white text-sm px-5 py-2 rounded-md opacity-0 transition-opacity z-50 whitespace-nowrap">
        Config berhasil disalin!
      </div>

      <div id="country-menu" class="hidden absolute right-0 mt-12 bg-white dark:bg-neutral-800/70 border border-neutral-300 dark:border-neutral-700 rounded-md shadow-md p-3 grid grid-cols-4 gap-3 w-64 max-h-80 overflow-y-auto scrollbar-hide z-40 text-center text-xs">
        PLACEHOLDER_BENDERA_NEGARA
      </div>
    </div>
    <!-- Tombol Dark Mode -->
    <button id="btn-darkmode" class="px-3 py-2 rounded-md bg-neutral-200 dark:bg-neutral-700/70 hover:bg-neutral-300 dark:hover:bg-neutral-600/70">☾</button>
    <!-- Donate -->
    <a href="https://trakteer.id/jhoy_id/tip" target="_blank" 
       class="px-3 py-2 rounded-md bg-indigo-500 hover:bg-indigo-600 text-white">💖</a>
  </div>
</nav>

<!-- Info -->
<div id="container-header" class="text-sm px-4 py-2 border-b border-neutral-200 dark:border-neutral-700/70">
...
<main class="container mx-auto px-4 py-6">
  ...
  <!-- Pagination -->
  <nav id="container-pagination" class="mt-8">
...
<script>
  ...
  function copyToClipboard(text) {
    ...
    showNotification("Config berhasil disalin!");
  }
  ...
  async function fetchMyIP() {
    try {
      ...
      document.getElementById("container-info-ip").innerText = "IP: " + (data.ip || "Unknown");
      document.getElementById("container-info-country").innerText = "Country: " + (data.country || "Unknown");
      document.getElementById("container-info-isp").innerText = "ISP: " + (data.asOrganization || "Unknown");
    } catch (e) {
      console.error("Failed to fetch IP info", e);
    }
  }

  // 🔹 Fungsi cek ping proxy (dengan warna hijau/kuning/merah)
  async function checkPing(ip, port, index) {
    ...
    try {
      ...
      if (!res.ok) {
        pingElement.innerText = \`❌ Timeout\`;
        pingElement.className = "text-xs font-semibold text-red-500 text-right";
        return;
      }
      ...
      pingElement.innerText = \`⚡ \${ms} ms • \${ip}:\${port}\`;
      pingElement.className = \`text-xs font-semibold \${colorClass} text-right\`;
    } catch (e) {
      pingElement.innerText = \`❌ Error\`;
      pingElement.className = "text-xs font-semibold text-red-500 text-right";
    }
  }

  document.addEventListener("DOMContentLoaded", () => {
    const btnMenu = document.getElementById("btn-country-menu");
    ...
    // 🔹 Jalankan cek ping untuk semua proxy
    const proxies = document.querySelectorAll("[id^=ping-]");
    proxies.forEach((el, i) => {
      const [ip, port] = el.innerText.replace("Idle ", "").split(":");
      checkPing(ip, port, i);
    });
  });
</script>  
...
class Document {
  ...
  for (let i = 0; i < this.proxies.length; i++) {
    ...
    proxyGroupElement += `
      <div class="lozad scale-95 mb-4 bg-white dark:bg-slate-800 transition-all duration-300 rounded-lg p-6 flex flex-col shadow-md hover:shadow-lg border border-slate-200 dark:border-slate-700 hover:scale-105">
        ...
        <div id="countryFlag" class="absolute -translate-y-11 -translate-x-2 border-4 border-white dark:border-slate-800 rounded-full overflow-hidden">
          <img width="48" src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" />
        </div>
        
        <!-- Ping Info -->
        <div class="flex-grow">
          <div id="ping-${i}" class="animate-pulse text-xs font-semibold text-slate-500 dark:text-slate-400 text-right">
        ...
        <div class="rounded-lg py-4 px-4 bg-slate-50 dark:bg-slate-700/50 flex-grow mt-4">
          <h5 class="font-bold text-lg text-slate-800 dark:text-slate-100 mb-1 overflow-x-scroll scrollbar-hide text-nowrap">${proxyData.org}</h5>
    ...
    for (let x = 0; x < proxyData.list.length; x++) {
      ...
      proxyGroupElement += `
        <button class="bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 rounded-md p-2 w-full text-white font-semibold transition-colors duration-200" onclick="copyToClipboard('${proxy}')">
          ${indexName[x]}
        </button>
      `;
...
buildCountryFlag() {
  ...
  for (const flag of new Set(flagList)) {
    flagElement += `
      <a href="/sub?cc=${flag}${proxyBankUrl ? "&proxy-list=" + proxyBankUrl : ""}" class="py-1">
  ...
  const pageButton = `
    <li>
      <button ${isDisabled ? "disabled" : ""}
        class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-400 dark:disabled:bg-slate-600 text-white font-semibold border-2 border-neutral-800 rounded-lg transition-colors"
